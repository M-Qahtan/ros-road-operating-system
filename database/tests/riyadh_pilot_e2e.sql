...
INSERT INTO road_events (
  ...
  '90000000-0000-4000-8000-000000000001', 'CLOSED', 'S3', 78, 0.940,
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM road_events WHERE id='90000000-0000-4000-8000-000000000001' AND status='CLOSED' AND severity='S3' AND closure_authorized_by IS NOT NULL) THEN
    RAISE EXCEPTION 'safe S3 closure invariant missing';
  END IF;
