...
| BRAIN-03 | Case projection | The governed read returns source versions only from the exact persisted snapshot after all five current owner receipts verify. Human-Safety exposes those revisions with an explicit `VERIFIED/WITHHELD/UNAVAILABLE` status and the dashboard labels unavailable values as withheld. | The compatibility numeric fields use zero rather than projected event/provenance values when no current governed snapshot exists. Live PostgreSQL execution is still required before claiming engine-backed source-version projection. |
...
| BRAIN-05 | Runtime health | Human-Safety reads now consume process readiness observations for PostgreSQL, Redis, active Object Storage, and required workers; missing/failed observation fails toward degraded or unavailable, and unhealthy state blocks resolution authorization. | Live dependency probes have not run in this executor. Operational acceptance still requires engine-backed observation and recovery evidence on the exact candidate. |
...
The restart-identity proof increment resumed from GitHub candidate `1304afbea70b98788863710e17bab451e630f97c`. GitHub resolved unchanged `main` at `8096312169dc7f769a45b419d5678b5bd5f461ad`, the branch twenty-one commits ahead and zero behind, with no branch PR or workflow run on the resume commit.

The runner now samples `pg_control_system().system_identifier` and `pg_postmaster_start_time()` immediately before and after the requested container restart. It rejects recovery unless the cluster identifier remains identical and the postmaster start timestamp changes, preventing either an unrestarted process or an accidental switch to a different database from satisfying the recovery checkpoint. Receipt schema v2 includes the surviving system identifier, recovered postmaster start time, and an explicit `restartVerified` marker only after this check and every SQL recovery assertion pass.

Fresh local verification passed 6/6 harness-definition tests and 602/602 workspace tests (API 529, dashboard 30, mobile 36, domain 7). All five TypeScript builds/no-emit checks and repository/composition/retention/negative gates passed, including 8/8 external-evidence logic tests. Docker remains unavailable and the preflight returned exit 127, so the identity query, restart, recovery assertions, and receipt were not executed against a live engine.
...
The five-source restart-recovery increment resumed from GitHub candidate `94805dda954602b737767993b7bceebc0059dbea`. Live GitHub comparison kept `main` at `8096312169dc7f769a45b419d5678b5bd5f461ad`, the branch twenty-three commits ahead and zero behind, with no branch PR or workflow run on the resume commit.

The first `REPEATABLE READ, READ ONLY` transaction after restart now verifies the complete authoritative state behind the persisted snapshot: exact RoadEvent CASE/SEVERITY receipts, Contact absence across both owned tables, the Evidence receipt, the two committed Human-Safety Indicator revisions, every version/digest stored in the snapshot, and the journal's exact snapshot digest. Missing or extra scoped receipts, a changed digest, an invented Contact state, or a broken journal link aborts the journey before the post-restart idempotent retry.

Fresh local verification passed 8/8 harness-definition tests and 604/604 workspace tests (API 531, dashboard 30, mobile 36, domain 7). All five TypeScript builds/no-emit checks and repository/composition/retention/negative gates passed, including 8/8 external-evidence logic tests. Docker remains unavailable and the explicit preflight returned exit 127, so these SQL assertions and the v3 receipt were not executed on a live PostgreSQL engine.
...
The post-restart forward-recovery increment resumed from GitHub candidate `22412000ad6e53e22196749bd2d483e13d488c95`. Live GitHub comparison kept `main` at `8096312169dc7f769a45b419d5678b5bd5f461ad`, the branch twenty-four commits ahead and zero behind, with no branch PR or workflow run on the resume commit.
...
The operational-health increment resumed from GitHub candidate `3a7e5553db685cabed3055b1b4f8d87cd22b0e97`. Live GitHub comparison kept `main` at `8096312169dc7f769a45b419d5678b5bd5f461ad`, the branch twenty-five commits ahead and zero behind, with no branch PR or workflow run on the resume commit.

Human-Safety list, detail, and action responses now consume bounded explicit runtime health observations instead of hardcoding connectivity and dependency health to `HEALTHY`. Persistent composition derives those observations from the existing bounded PostgreSQL, Redis, active Object Storage, and required-worker checks. Missing observation is degraded/unavailable; a probe exception or worker failure is unavailable. A supervisor may authorize high-risk resolution only when the currently observed connectivity and dependencies are both healthy and Evidence is trusted. Failure returns `OPERATIONAL_HEALTH_UNVERIFIED` without a closure authorization write, while human takeover and escalation remain available. Successful authorization remains a separate human record and does not close the RoadEvent or grant the recommendation execution authority.

Fresh local verification passed 13/13 focused Human-Safety API tests and 608/608 workspace tests (API 535, dashboard 30, mobile 36, domain 7). All five TypeScript builds/no-emit checks and repository/composition/retention/negative gates passed, including 8/8 external-evidence logic tests. Docker remains unavailable and the explicit preflight returned exit 127, so no live dependency observation, PostgreSQL journey, or v3 receipt was produced.

Result: **HARDCODED HUMAN-SAFETY HEALTH WAS REMOVED AND UNHEALTHY CLOSURE AUTHORIZATION NOW FAILS CLOSED; LIVE RUNTIME EVIDENCE REMAINS OPEN.** The next single handoff is to project the verified snapshot's authoritative source revisions into the Human-Safety case view and withhold those claims when the governed snapshot is unavailable.

The persisted closure-snapshot binding increment resumed from GitHub candidate `92fd7e9f65bd4c3ecf9ce32ef569012bd3627e28`. Live GitHub comparison kept `main` at `8096312169dc7f769a45b419d5678b5bd5f461ad`, the branch twenty-eight commits ahead and zero behind, with no branch PR or workflow run on the resume commit.

After the existing Human-Safety read gate verifies a current governed recommendation and exact source snapshot, the authorization command now copies that snapshot's input version and digest into the RoadEvent closure authorization. The domain validates and preserves the pair; PostgreSQL create/update/read paths persist it. Migration `0022_road_event_closure_snapshot_binding.sql` requires the pair to be wholly absent or valid and uses a composite foreign key over Tenant + Purpose + case + input version + digest, preventing a stored authorization from rebinding to another scope, version, or snapshot digest. Legacy unbound authorization rows remain readable but gain no governed-snapshot claim.

Fresh local verification passed 41/41 focused domain/application/Human-Safety/PostgreSQL tests and 614/614 workspace tests (API 539, dashboard 31, mobile 36, domain 8). All five TypeScript builds/no-emit checks and repository/composition/retention/negative gates passed, including 8/8 external-evidence logic tests. The migration and transaction behavior were not executed on a live PostgreSQL engine in this executor, and no external REL-013 archive receipt was produced.
...
The closure/source concurrency increment resumed from GitHub candidate `21612fb4e1667aefd45827609effb1ffbdb443cf`. Live GitHub comparison kept `main` at `8096312169dc7f769a45b419d5678b5bd5f461ad`, the branch thirty-one commits ahead and zero behind, with no branch PR or workflow run on the resume commit.

The Human-Safety Indicator command already serialized on the exact RoadEvent row; it now rejects an append after that locked row is `CLOSED`, preventing a closure-first interleaving from accepting later structured source state. The disposable journey adds a real two-client overlap for the opposite ordering: the source correction holds the RoadEvent lock, the closure waits, and after the correction commits the closure must revalidate and fail with `SOURCE_SNAPSHOT_CHANGED`. It then requires durable `RECOVERY / version 2 / Indicator revision 3`, proving one committed participant and no closure mutation. A test-only advisory lock makes the overlap deterministic but does not provide production safety; both production commands rely on the RoadEvent row lock and serializable transactions.

The runner transfers the exact winner/loser disposition through a unique empty regular proof file. Local receipt schema v4 includes `closureRaceVerified`, the `SOURCE_UPDATE` winner, and the `SOURCE_SNAPSHOT_CHANGED` closure result only after every migration, restart, recovery, and race assertion passes. The race script is included in the candidate manifest digest. `externalArchiveReceipt` remains null, so even a live local v4 receipt is not REL-013 evidence.
...
The post-restart case-isolation increment resumed from GitHub candidate `bb2f7a05579d5232cf6bf452369c4ca36a135130`, with `main` at `8096312169dc7f769a45b419d5678b5bd5f461ad`, the review branch forty-six commits ahead and zero behind, and no branch PR or workflow run. The Contact recovery journey now supplies a foreign case while retaining the valid tenant and purpose. The serializable parent lookup must fail before the otherwise-valid Contact update can execute, and the complete Contact/Revision/Audit/Outbox state must remain byte-for-byte equivalent to the recovered state. Receipt schema v14 records the rejected case retry only after that persistence check. This defines the full `Tenant + Purpose + Case` isolation boundary without granting execution authority; live PostgreSQL proof and the external REL-013 archive remain open.

The post-restart parent-version increment resumed from GitHub candidate `04d3799324f5df41e1ed6e4c5cbc5ea014521844`, while live GitHub still showed `main` at `8096312169dc7f769a45b419d5678b5bd5f461ad`, the branch forty-seven commits ahead and zero behind, and no branch PR or workflow run. The journey now locks the exact RoadEvent but asserts the command's stale expected parent version before touching a current Contact session. A mismatch must abort the transaction and preserve Contact, revision, Audit, and Outbox state exactly. Receipt schema v15 binds the rejection and unchanged state, completing the command's `Tenant + Purpose + Case + Version` boundary definition. It does not grant recommendation execution, dispatch, closure, or activation authority; live PostgreSQL proof and REL-013 external archival remain open.

The post-restart terminal-state increment resumed from GitHub candidate `d695d7fb56b1bb85d19c61bf94fde0b4fef172ad`, with `main` unchanged, the review branch forty-eight commits ahead and zero behind, and no branch PR or workflow run. After the closure-winner race and the later PostgreSQL restart, the journey locks that exact RoadEvent and requires persisted `CLOSED` to reject a Contact mutation before session access. The case must remain `CLOSED / event 3 / session 1 / contact 1 / audit 0 / cancelled 0 / pending 1`; receipt schema v16 binds that terminal rejection and state. The assertion neither reopens the incident nor grants dispatch, recommendation execution, collection, or activation authority. Live PostgreSQL proof and REL-013 external archival remain open.

The closed-parent delivery-fence increment resumed from GitHub candidate `c8190688fb0a537d191b0888f577736c32785d03`, with `main` unchanged, the review branch forty-nine commits ahead and zero behind, and no branch PR or workflow run. The Contact-owned PostgreSQL outbox worker now reads the exact tenant/case RoadEvent boundary before claiming pending work and rechecks it before creating the short delivery reservation. An already-committed `CLOSED` parent is reported as `CANCELLED`, and the provider callback is not entered. This is a recommendation-safe dispatch fence, not dispatch authority: it does not send a message or prove the concurrent interval after reservation. The next integration step is to exercise this production SQL against the disposable PostgreSQL journey and bind the no-provider result to its next engine receipt; live proof and REL-013 external archival remain open.
