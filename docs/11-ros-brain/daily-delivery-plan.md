...
### Arabic operator workflow for governed ambiguous retries

| Field | Current record |
|---|---|
| Resume point | On 2026-09-21, GitHub candidate `39b3878c89d1423a50d4766b677224ce5d3bf308` was one hundred and nine commits ahead of current `main` at `3255a94a7f78607014a410e083174483fa2c2c2f` and zero behind. Its tree matched the clean local candidate, both delivery documents remained present, and no overlapping test process, branch pull request, or candidate workflow run existed. The approved cadence remains hourly. |
| Added behavior | An ambiguous closure authorization or RoadEvent transition now appears in the Arabic dashboard as an incident-scoped review panel showing the original incident, action, and expected version without exposing its idempotency key or request reason. A stale outcome offers only an authenticated incident-and-Timeline verification. The exact original resend appears only after that read confirms the same revision and current authority; the browser then invokes the controller's preserved operation identity rather than creating a new command. |
| Fail-closed acceptance | Before verification the resend control must not exist. A matching authenticated refresh may expose it, success must remove the panel, and any changed incident revision, terminal state, role, authorization prerequisite, or other eligibility change must render a disabled outcome with neither verification nor resend action. |
| Local evidence | The focused operations-dashboard build and suite passed 42 of 42. Authenticated browser tests prove the Arabic stale, verified, successful, and changed-revision states while retaining exact idempotency-key reuse. The full workspace passed 666 API, 42 dashboard, 36 mobile, and 8 domain tests (752 total), plus 32 perception, benchmark, adapter-certification, and epistemic-coverage contract checks. Build, no-emit TypeScript, repository/runtime composition, retention, negative-gate, archive conditional-write, and eight external-evidence policy checks passed. The PostgreSQL journey exited 127 before execution because neither Docker nor Podman is installed; no live `v33` receipt is claimed. |
| Safety limits | This UI is a human-reviewed recovery path for an in-memory ambiguous operation, not evidence that the first request committed or failed. It exposes no key or reason, survives no reload, and grants no reopening, collection, severity reduction, dispatch, activation, provider call, or autonomous authority. `RECOMMENDATION_ONLY`, `SHADOW_ONLY`, and `activationAuthorized=false` remain unchanged. Local evidence does not satisfy REL-013 external immutable archival. |
...
### Recovered authorization closes once and remains terminal after reconciliation

| Field | Current record |
|---|---|
| Resume point | On 2026-09-22, GitHub candidate `f20b65eec48f53571acfb5193222981ff6fb35bc` was one hundred and sixteen commits ahead of current `main` at `3255a94a7f78607014a410e083174483fa2c2c2f` and zero behind. Its tree matched the clean local candidate, both delivery documents remained present, and no overlapping test process, branch pull request, or candidate workflow run existed. The approved cadence remains hourly. |
| Added evidence | The authenticated recovery journey now submits the supervisor-confirmed `CLOSED` transition once from recovered server version 13. The server advances the incident to terminal version 14, consumes the closure authorization, appends `road_event.closed`, and a subsequent authenticated queue reconciliation plus re-selection reads that terminal state and both append-only records. |
| Fail-closed acceptance | The closure request must carry expected version 13 and occur exactly once. After reconciliation, the incident must be `CLOSED@14` with `closureAuthorization=null`; Timeline must contain exactly the authorization and closure records in order. Reopening or issuing a new authorization must fail locally before HTTP. |
| Local evidence | The focused operations-dashboard build and suite passed 46 of 46. The journey observed one authorization POST and one transition POST, five authenticated Timeline reads across the full recovery lifecycle, and four queue reads including the post-closure reconciliation. The final view contained `road_event.closure_authorized` followed by `road_event.closed`; both critical control predicates were false and rejected commands emitted no request. The full workspace passed 666 API, 46 dashboard, 36 mobile, and 8 domain tests (756 total), plus 32 perception, benchmark, adapter-certification, and epistemic-coverage contract checks. Build, no-emit TypeScript, repository/runtime composition, retention, negative-gate, archive conditional-write, and eight external-evidence policy checks passed. The PostgreSQL journey exited 127 before execution because neither Docker nor Podman is installed; no live `v33` receipt is claimed. |
| Safety limits | This browser/API contract proof does not establish a live PostgreSQL transaction or external immutable archive. It grants no autonomous closure, reopening, severity reduction, dispatch, activation, collection, provider call, or operational authority. Tenant, purpose, role, exact-version, human confirmation, and append-only audit boundaries remain authoritative. `RECOMMENDATION_ONLY`, `SHADOW_ONLY`, and `activationAuthorized=false` remain unchanged. Local evidence does not satisfy REL-013 external immutable archival. |
