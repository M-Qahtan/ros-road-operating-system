...
| HSE-14 | P1 | clock skew corrupts authorization, replay TTL, deadlines or event order | strict timestamp chronology, bounded skew and expiry | skew threshold and rejected chronology audit | HUMAN_REVIEW or quarantine | contract tests; #30 clock-skew fixtures | Signal Integrity Lead |
...
Replay evidence stores only the replay policy version, digest-scoped identity, expiry, and consume result. Raw replay tokens are prohibited from logs, traces, metrics, audit events, and artifacts.
