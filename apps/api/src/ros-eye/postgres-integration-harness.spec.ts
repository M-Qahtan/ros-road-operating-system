...
const cognitiveClosureRecovery = readFileSync('scripts/run-postgres-cognitive-closure-recovery.sh', 'utf8');
const closureAuthorizationRead = readFileSync('scripts/run-postgres-closure-authorization-read.sh', 'utf8');
const closureReauthorizationRecovery = readFileSync(
  'scripts/run-postgres-closure-reauthorization-recovery.sh',
  'utf8',
);
const closureReauthorizationFinalize = readFileSync(
...
test('live journey receipt is bound to a clean candidate and emitted only after success', () => {
  ...
  assert.match(localHarness, /ros-brain\.local-postgres-journey-receipt\.v33/);
...
test('v33 receipt consumes contact races, rollback and exact forward retry proof', () => {
...
test('v33 recovers cognitive closure drift rejection state after restart', () => {
  ...
  assert.match(localHarness, /cognitive_closure_state_after_restart/);
  assert.match(localHarness, /cognitive_closure_state_after_restart" != "\$cognitive_closure_state_before_restart/);
...
test('v33 retries cognitive closure after restart and rejects without durable writes', () => {
  ...
  assert.match(cognitiveClosureRecovery, /GET DIAGNOSTICS changed = ROW_COUNT/);
  assert.match(cognitiveClosureRecovery, /road_event_closure_authorization_journal authorization_journal/);
  assert.match(cognitiveClosureRecovery, /Post-restart cognitive drift permitted closure/);
  assert.match(cognitiveClosureRecovery, /Post-restart cognitive closure retry changed the write-set/);
  assert.match(cognitiveClosureRecovery, /Post-restart cognitive closure retry rewrote authorization history/);
  assert.match(localHarness, /bash scripts\/run-postgres-cognitive-closure-recovery\.sh/);
  assert.match(localHarness, /cognitiveClosurePostRestartRetry/);
  assert.match(localHarness, /cognitiveClosurePostRestartWriteSet/);
  assert.match(localHarness, /cognitiveClosurePostRestartAuthorizationHistory/);
  assert.ok(
...
test('v33 persists and consumes one exact append-only closure authorization journal row across restart', () => {
  ...
  assert.match(cognitiveClosureDrift, /RoadEvent closure authorization journal is append-only/);
  assert.match(cognitiveClosureDrift, /road_event_closure_authorization_journal authorization_journal/);
  assert.match(localHarness, /CLOSURE_AUTHORIZATION_JOURNAL.*UPDATE_DELETE_REJECTED/s);
  assert.match(localHarness, /closure_authorization_journal_state_before_restart/);
  assert.match(localHarness, /closure_authorization_journal_state_after_restart/);
...
test('v33 withholds a mismatched closure authorization read after restart without durable writes', () => {
  ...
  assert.match(closureAuthorizationRead, /ROLLBACK TO SAVEPOINT mismatched_read/);
  assert.match(closureAuthorizationRead, /Closure authorization read proof changed durable state/);
  assert.match(localHarness, /bash scripts\/run-postgres-closure-authorization-read\.sh/);
  assert.ok(
    localHarness.indexOf('wait_for_postgres "after contact recovery restart"') <
      localHarness.indexOf('bash scripts/run-postgres-closure-authorization-read.sh'),
  );
...
test('v33 withholds a closure authorization whose independent journal row is missing', () => {
...
test('v33 rolls back stale closure and commits only exact-revision replacement authorization', () => {
  assert.match(closureReauthorizationRecovery, /BEGIN TRANSACTION ISOLATION LEVEL SERIALIZABLE/);
  assert.match(closureReauthorizationRecovery, /SAVEPOINT stale_closure_attempt/);
  assert.match(closureReauthorizationRecovery, /AND version=8/);
  assert.match(closureReauthorizationRecovery, /ROLLBACK TO SAVEPOINT stale_closure_attempt/);
  assert.match(closureReauthorizationRecovery, /Stale revision-8 closure changed the durable write-set/);
  assert.match(closureReauthorizationRecovery, /Revision 9 exposed the historical revision-8 authorization/);
  assert.match(closureReauthorizationRecovery, /AND version=9/);
  assert.match(closureReauthorizationRecovery, /journal_versions<>ARRAY\[8,10\]/);
  assert.match(closureReauthorizationRecovery, /audit_versions<>ARRAY\[8,10\]/);
  assert.match(closureReauthorizationRecovery, /current_historical_hash<>baseline\.historical_journal_hash/);
  assert.match(localHarness, /bash scripts\/run-postgres-closure-reauthorization-recovery\.sh/);
  assert.match(localHarness, /closureReauthorizationRecoveryVerified: true/);
  assert.match(localHarness, /closureReauthorizationStaleRollback/);
  assert.match(localHarness, /closureReauthorizationRefresh/);
  assert.match(localHarness, /closureReauthorizationReplacement/);
  assert.match(localHarness, /closureReauthorizationHistory/);
  assert.match(localHarness, /closureReauthorizationCurrentVersion/);
  assert.match(localHarness, /ros-brain\.local-postgres-journey-receipt\.v33/);
});

test('v33 requires replacement authorization to survive a distinct PostgreSQL restart', () => {
  assert.match(localHarness, /closure_reauthorization_state\(\)/);
  assert.match(localHarness, /closure_reauthorization_identity_before_restart/);
  assert.match(localHarness, /closure_reauthorization_state_before_restart/);
  assert.match(localHarness, /restart -- "\$container_name"/);
  assert.match(localHarness, /wait_for_postgres "after closure reauthorization restart"/);
  assert.match(localHarness, /closure_reauthorization_identity_after_restart/);
  assert.match(localHarness, /closure_reauthorization_state_after_restart/);
  assert.match(localHarness, /RECOVERY\\\|10\\\|AUTHORIZED\\\|8,10\\\|8,10\\\|1/);
  assert.match(localHarness, /Closure reauthorization did not survive PostgreSQL restart exactly/);
  assert.match(localHarness, /closureReauthorizationRestartVerified: true/);
  assert.match(localHarness, /closureReauthorizationStateBeforeRestart/);
  assert.match(localHarness, /closureReauthorizationStateAfterRestart/);
  assert.match(localHarness, /closureReauthorizationPostmasterStartedAtBeforeRestart/);
  assert.match(localHarness, /closureReauthorizationPostmasterStartedAtAfterRestart/);
  assert.match(localHarness, /ros-brain\.local-postgres-journey-receipt\.v33/);
});

test('v33 consumes exact authorization once after restart and preserves historical evidence', () => {
  assert.match(closureReauthorizationFinalize, /BEGIN TRANSACTION ISOLATION LEVEL SERIALIZABLE/);
  assert.match(closureReauthorizationFinalize, /AND version=8/);
  assert.match(closureReauthorizationFinalize, /Historical closure retry changed the durable write-set/);
  assert.match(closureReauthorizationFinalize, /AND event\.version=10/);
  assert.match(closureReauthorizationFinalize, /road_event_closure_authorization_journal journal/);
  assert.match(closureReauthorizationFinalize, /ORDER BY cognitive\.input_version DESC LIMIT 1/);
  assert.match(closureReauthorizationFinalize, /SET status='CLOSED', version=11/);
  assert.match(closureReauthorizationFinalize, /road_event\.closed/);
  assert.match(closureReauthorizationFinalize, /RoadEventClosed/);
  assert.match(closureReauthorizationFinalize, /Duplicate revision-10 closure unexpectedly wrote again/);
  assert.match(closureReauthorizationFinalize, /journal_versions<>ARRAY\[8,10\]/);
  assert.match(closureReauthorizationFinalize, /current_authorization_count<>0/);
  assert.match(closureReauthorizationFinalize, /historical_journal_hash<>baseline\.historical_journal_hash/);
  assert.ok(
    localHarness.indexOf('wait_for_postgres "after closure reauthorization restart"') <
      localHarness.indexOf('bash scripts/run-postgres-closure-reauthorization-finalize.sh'),
  );
  assert.match(localHarness, /postRestartClosureFinalizationVerified: true/);
  assert.match(localHarness, /postRestartHistoricalAuthorization/);
  assert.match(localHarness, /postRestartExactAuthorization/);
  assert.match(localHarness, /postRestartDuplicateClosureRetry/);
  assert.match(localHarness, /postRestartClosureAuthorizationHistory/);
  assert.match(localHarness, /postRestartClosureFinalState/);
  assert.match(localHarness, /ros-brain\.local-postgres-journey-receipt\.v33/);
});

test('v33 preserves terminal closure and withholds consumed authorization across a distinct restart', () => {
  assert.match(localHarness, /closure_finalized_state\(\)/);
  assert.match(localHarness, /closure_finalization_identity_before_restart/);
  assert.match(localHarness, /closure_finalization_state_before_restart/);
  assert.match(localHarness, /restart -- "\$container_name"/);
  assert.match(localHarness, /wait_for_postgres "after closure finalization restart"/);
  assert.match(localHarness, /closure_finalization_identity_after_restart/);
  assert.match(localHarness, /closure_finalization_state_after_restart/);
  assert.match(localHarness, /CLOSED\\\|11\\\|WITHHELD\\\|8,10\\\|8,10\\\|1\\\|1\\\|\[a-f0-9\]\{32\}/);
  assert.match(localHarness, /Terminal closure did not survive PostgreSQL restart exactly/);
  assert.match(localHarness, /terminalClosureRestartVerified: true/);
  assert.match(localHarness, /terminalClosureStateBeforeRestart/);
  assert.match(localHarness, /terminalClosureStateAfterRestart/);
  assert.match(localHarness, /terminalClosurePostmasterStartedAtBeforeRestart/);
  assert.match(localHarness, /terminalClosurePostmasterStartedAtAfterRestart/);
  assert.match(localHarness, /ros-brain\.local-postgres-journey-receipt\.v33/);
});
