...
import {
 ...
 type BreakGlassGrantTransaction, type BreakGlassLease, type BreakGlassLeaseLookup, type ConsentAuthorityPort, type ConsentGrantReceipt,
 type PrivacyClockPort, type RecommendationApprovalReceipt, type RecommendationAuthorityPort, type RecommendationExecutionRequest
...
class MemoryClock implements PrivacyClockPort{
 constructor(public current='2026-07-29T20:00:00.000Z'){}
 async now(){return this.current}
}
...
class MemoryGrants implements BreakGlassGrantRepositoryPort,BreakGlassGrantTransaction{
 ...
 authoritativeLease:BreakGlassLease|null=createLease(); failAlert=false; failAudit=false; abuseResult:'ALLOW'|'RATE_LIMIT'|'ANOMALY_REVIEW'='ALLOW';
 invalidateAfterAudit:'NONE'|'EXPIRE'|'REVOKE'|'REVIEW'='NONE';
 async transaction<T>(work:(tx:BreakGlassGrantTransaction)=>Promise<T>):Promise<T>{return work(this)}
 async findLeaseForUpdate(input:BreakGlassLeaseLookup){const l=this.authoritativeLease;return l!==null&&l.tenantId===input.tenantId&&l.caseId===input.caseId&&l.leaseId===input.leaseId&&l.actorId===input.actorId&&l.role===input.role&&l.purpose===input.purpose?l:null}
 ...
 async appendAudit(input:BreakGlassGrantInput&{alertReceiptId:string}){if(this.failAudit||!this.alerts.has(input.alertReceiptId))return{status:'FAILED' as const,eventId:null};const eventId=`audit-${input.grantId}`;const existed=this.audits.has(eventId);this.audits.add(eventId);const l=this.authoritativeLease;if(l!==null){if(this.invalidateAfterAudit==='REVOKE')this.authoritativeLease={...l,revokedAt:input.occurredAt};if(this.invalidateAfterAudit==='REVIEW')this.authoritativeLease={...l,reviewedAt:input.occurredAt};if(this.invalidateAfterAudit==='EXPIRE')this.authoritativeLease={...l,expiresAt:input.occurredAt};}return{status:existed?'IDEMPOTENT' as const:'APPENDED' as const,eventId}}
 async finalizeGrant(input:BreakGlassGrantInput&{alertReceiptId:string;auditEventId:string}){if(!this.abuse.has(input.grantId)||!this.alerts.has(input.alertReceiptId)||!this.audits.has(input.auditEventId))return{status:'CONFLICT' as const,receipt:null};const existing=this.receipts.get(input.grantId);if(existing)return{status:'IDEMPOTENT' as const,receipt:existing};const receipt:BreakGlassGrantReceipt={...input,alertReceiptId:input.alertReceiptId,auditEventId:input.auditEventId,status:'AUTHORIZED'};this.receipts.set(input.grantId,receipt);return{status:'AUTHORIZED' as const,receipt}}
}
...
function createLease(){const value=createBreakGlassLease({tenantId:'tenant-riyadh',caseId:'case-001',leaseId:'lease-001',actorId:'operator-001',role:'SAFETY_OPERATOR',purpose:'OPERATOR_REVIEW',reasonCode:'immediate_safety_review',issuedAt:'2026-07-29T20:00:00.000Z',durationMs:60_000});assert.ok(value);return value}
function lease(){return createLease()}
function raw(overrides:Partial<AccessRequest>={}):AccessRequest{return{...base,dataKind:'EVIDENCE_RAW',breakGlass:lease(),...overrides}}
function service(repo=new MemoryGrants(),consent=new MemoryConsent(),clock=new MemoryClock()){return new PrivacyAccessOrchestrator(repo,ids,consent,clock)}
...
test('authoritative consent is bound to session subject purpose data action sequence and trusted time',async()=>{
 assert.equal((await service().authorizeAccess(base)).allowed,true);
 for(const request of [{...base,consentGrantId:'fabricated-001'},{...base,sessionId:'session-other'},{...base,subjectId:'subject-other'},{...base,lifecycle:'CONSENT_PENDING' as const},{...base,lifecycle:'LANGUAGE_SELECTION' as const}])assert.equal((await service().authorizeAccess(request)).allowed,false);
 ...
 const missing=new MemoryConsent();missing.grant=null;assert.equal((await service(new MemoryGrants(),missing).authorizeAccess(base)).reasonCode,'consent_record_missing');
 const expiredClock=new MemoryClock('2026-07-29T20:11:00.000Z');assert.equal((await service(new MemoryGrants(),new MemoryConsent(),expiredClock).authorizeAccess({...base,now:'2026-07-29T20:00:00.000Z'})).allowed,false);
});

test('consent expiry revocation and incomplete sequence deny',async()=>{
 const consent=new MemoryConsent();consent.grant={...consent.grant!,expiresAt:'2026-07-29T19:59:59.000Z'};assert.equal((await service(new MemoryGrants(),consent).authorizeAccess(base)).allowed,false);
 consent.grant={...consent.grant!,expiresAt:'2026-07-29T20:05:00.000Z',revokedAt:'2026-07-29T19:59:30.000Z',status:'REVOKED'};assert.equal((await service(new MemoryGrants(),consent).authorizeAccess(base)).allowed,false);
 consent.grant={...consent.grant!,revokedAt:null,status:'ACTIVE',contactState:'CONTACTING',disclosureLanguage:''};assert.equal((await service(new MemoryGrants(),consent).authorizeAccess(base)).allowed,false);
});

test('durable alert audit abuse authoritative lease and finalization are all required before FULL access',async()=>{
 const repo=new MemoryGrants();const result=await service(repo).authorizeAccess(raw());
 assert.equal(result.mode,'FULL');assert.ok(result.receiptId);assert.equal(repo.alerts.size,1);assert.equal(repo.audits.size,1);assert.equal(repo.abuse.size,1);assert.equal(repo.receipts.size,1);
});
...
test('authoritative lease expiry review and revocation during orchestration deny finalization',async()=>{
 for(const invalidation of ['EXPIRE','REVIEW','REVOKE'] as const){const repo=new MemoryGrants();repo.invalidateAfterAudit=invalidation;const result=await service(repo).authorizeAccess(raw());assert.equal(result.reasonCode,'break_glass_lease_invalidated');assert.equal(repo.receipts.size,0);}
});

test('trusted clock denies backdated request and duplicate retry after invalidation',async()=>{
 const repo=new MemoryGrants();const clock=new MemoryClock('2026-07-29T20:01:01.000Z');assert.equal((await service(repo,new MemoryConsent(),clock).authorizeAccess(raw({now:'2026-07-29T20:00:00.000Z'}))).mode,'DENY');
 repo.authoritativeLease={...lease(),revokedAt:'2026-07-29T20:00:30.000Z'};clock.current='2026-07-29T20:00:40.000Z';assert.equal((await service(repo,new MemoryConsent(),clock).authorizeAccess(raw())).mode,'DENY');assert.equal(repo.receipts.size,0);
});

test('invented receipts and cross actor case tenant fail closed',async()=>{
 ...
 for(const request of [raw({actorId:'operator-002'}),raw({actorCaseId:'case-other'}),raw({actorTenantId:'tenant-other'})])assert.equal((await service().authorizeAccess(request)).mode,'DENY');
});
...
test('break glass maximum duration and legal hold boundaries are enforced',async()=>{
 ...
 assert.equal((await service().authorizeAccess(retention)).allowed,true);assert.equal((await service().authorizeAccess({...base,lifecycle:'LEGAL_HOLD'})).allowed,false);
});
...
test('authoritative human approval prevents model self-authorization and replay',async()=>{
 ...
 assert.equal(await orchestrator.authorize(request),true);assert.equal(await orchestrator.authorize({...request,approvalId:'self-issued'}),false);
...
test('threat model has owner control test and no accepted P0',()=>{
 assert.ok(THREAT_CONTROL_MATRIX.length>=6);for(const row of THREAT_CONTROL_MATRIX){assert.ok(row.owner);assert.ok(row.control);assert.ok(row.test);assert.notEqual(row.residualRisk,'P0');}
});
