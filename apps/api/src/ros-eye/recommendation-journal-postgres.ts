...
export type RecommendationJournalReason = 'STORED' | 'EXACT_REPLAY' | 'DUPLICATE_INPUT' | 'SNAPSHOT_NOT_FOUND' |
  'BINDING_INVALID' | 'COGNITIVE_SNAPSHOT_NOT_FOUND' | 'COGNITIVE_BINDING_INVALID' |
  'COGNITIVE_ABSTENTION_REQUIRED' | 'GOVERNANCE_INVALID' | 'RECOMMENDATION_INVALID';
...
interface JournalRow extends ContactSqlRow {
  ...
  readonly cognitive_revision: number;
...
export const POSTGRES_RECOMMENDATION_JOURNAL_SQL = Object.freeze({
  readExact: `SELECT deterministic_fingerprint, source_snapshot_digest,
      cognitive_snapshot_policy_version, cognitive_revision, cognitive_digest,
      cognitive_requires_abstention, recommendation, binding
  ...
  insert: `INSERT INTO ros_eye_safety_fusion_recommendation_journal (
      ...
      $10, $11, $12, $13,
      'RECOMMENDATION_ONLY', 'SHADOW_ONLY', false, 'PENDING', $14, $15, $16::jsonb, $17::jsonb
...
export class PostgresRecommendationJournal {
  ...
  async appendWithin(connection: ContactSqlConnectionPort, request: AppendRecommendationRequest): Promise<RecommendationJournalResult> {
    ...
    const cognitive = await this.cognitiveSnapshots.readWithin(connection, request, request.recommendation.inputVersion);
    if (cognitive === null) return result('REJECTED', 'COGNITIVE_SNAPSHOT_NOT_FOUND');
    if (!validCognitiveReceipt(cognitive, snapshot.snapshotDigest, snapshot.capturedAt)) {
      return result('REJECTED', 'COGNITIVE_BINDING_INVALID');
    }
    if (cognitive.cognitive.requiresAbstention) return result('REJECTED', 'COGNITIVE_ABSTENTION_REQUIRED');

    const existing = await readExact(connection, request);
    if (existing !== null) return sameEntry(existing, request, cognitive) ? result('IDEMPOTENT', 'EXACT_REPLAY') : result('CONFLICT', 'DUPLICATE_INPUT');

    const inserted = await connection.query(POSTGRES_RECOMMENDATION_JOURNAL_SQL.insert, values(request, cognitive));
    if (inserted.rowCount === 1) return result('CREATED', 'STORED');
    const winner = await readExact(connection, request);
    return winner !== null && sameEntry(winner, request, cognitive)
...
function values(request: AppendRecommendationRequest, cognitive: CognitiveInputSnapshotReceipt): readonly unknown[] {
  ...
  return [
    ...
    recommendation.evaluatedAt, recommendation.deterministicFingerprint,
    cognitive.policyVersion, cognitive.cognitive.revision, cognitive.cognitive.digest,
    cognitive.cognitive.requiresAbstention, recommendation.ruleSetVersion,
...
function validCognitiveReceipt(receipt: CognitiveInputSnapshotReceipt, baseDigest: string, capturedAt: string): boolean {
  return receipt.policyVersion === COGNITIVE_INPUT_SNAPSHOT_POLICY_VERSION &&
    receipt.baseSnapshotDigest === baseDigest && receipt.capturedAt === capturedAt;
}

function sameEntry(row: JournalRow, request: AppendRecommendationRequest, cognitive: CognitiveInputSnapshotReceipt): boolean {
  return row.deterministic_fingerprint === request.recommendation.deterministicFingerprint &&
    ...
    row.cognitive_snapshot_policy_version === cognitive.policyVersion &&
    Number(row.cognitive_revision) === cognitive.cognitive.revision &&
    row.cognitive_digest === cognitive.cognitive.digest &&
    row.cognitive_requires_abstention === cognitive.cognitive.requiresAbstention &&
