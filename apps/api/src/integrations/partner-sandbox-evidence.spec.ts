...
interface BundleFixture {
  ...
  receiptFiles: Array<{ path: string; sha256: string; sizeBytes: number }>;
}
...
test('trusted byte-verified sandbox evidence is only verified for external review', async () => {
  const raw = bundle();
  ...
  await withReceiptRoot(raw, async (root) => {
    ...
    assert.equal(decision.status, 'VERIFIED_FOR_EXTERNAL_REVIEW');
    ...
    assert.equal(decision.networkCalls, 8);
    assert.deepEqual(decision.blockingReasons, []);
  });
});

test('metadata-only sandbox evidence can never verify itself', () => {
...
test('tampered receipt bytes are rejected before evaluation', async () => {
  ...
  const parsed = parsePartnerSandboxEvidenceBundle(raw);
  await withReceiptRoot(raw, async (root) => {
    await writeFile(join(root, raw.receiptFiles[0]!.path), Buffer.from('tampered\n', 'utf8'));
    await assert.rejects(verifyPartnerSandboxEvidence(parsed, root, expected()), /size mismatch|SHA-256 mismatch/);
  });
});

test('no network calls or duplicate logical actions remain NO_GO', async () => {
  ...
  assert.match(noCallsDecision.blockingReasons.join(' | '), /no actual approved sandbox network call/);
  ...
  assert.match(duplicateDecision.blockingReasons.join(' | '), /duplicate logical actions/);
});

test('callback, outage, minimum-necessary and status/cancel claims fail closed when absent', () => {
  const fields = [
...
test('unknown fields and duplicate receipt paths are rejected', () => {
  ...
  const duplicate = bundle();
  duplicate.receiptFiles.push({ ...duplicate.receiptFiles[0]! });
