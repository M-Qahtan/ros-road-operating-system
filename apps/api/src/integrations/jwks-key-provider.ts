...
interface JwksDocument { readonly keys?: unknown; }

export interface JwksDocumentFetcherPort { fetchJwks(): Promise<unknown>; }
...
function nonNegativeInteger(value: number, field: string): number {
  if (!Number.isSafeInteger(value) || value < 0) throw new JwksProviderError(`${field} must be a non-negative safe integer`);
  return value;
}
...
function toRs256Key(record: JwksKeyRecord): { readonly kid: string; readonly key: KeyObject } | undefined {
  ...
  try {
    return { kid, key: createPublicKey({ key: { kty: 'RSA', n, e }, format: 'jwk' }) };
  } catch {
...
function parseJwks(document: unknown, maximumKeys: number): Map<string, KeyObject> {
  ...
  if (!Array.isArray(keys) || keys.length === 0) throw new JwksProviderError('JWKS document must contain a non-empty keys array');
  if (keys.length > maximumKeys) throw new JwksProviderError(`JWKS document exceeds the configured ${maximumKeys}-key limit`);
  ...
  for (const raw of keys) {
    ...
    if (parsed.has(candidate.kid)) throw new JwksProviderError(`JWKS contains duplicate kid ${candidate.kid}`);
    parsed.set(candidate.kid, candidate.key);
  }
  if (parsed.size === 0) throw new JwksProviderError('JWKS contains no usable RS256 verification keys');
  return parsed;
}
...
export class CachedJwksRs256KeyProvider implements OidcVerificationKeyProviderPort {
  ...
  constructor(private readonly fetcher: JwksDocumentFetcherPort, options: CachedJwksProviderOptions) {
    this.cacheTtlSeconds = positiveInteger(options.cacheTtlSeconds, 'cacheTtlSeconds');
    this.minimumRefreshIntervalSeconds = nonNegativeInteger(options.minimumRefreshIntervalSeconds ?? 5, 'minimumRefreshIntervalSeconds');
  ...
  async resolveRs256PublicKey(rawKid: string): Promise<KeyObject | undefined> {
    ...
    if (this.keys.size === 0 || now >= this.expiresAtEpochSeconds) await this.refresh(now);
    const cached = this.keys.get(kid);
    if (cached !== undefined) return cached;
    if (now - this.lastRefreshAttemptEpochSeconds < this.minimumRefreshIntervalSeconds) return undefined;
  ...
  private readClock(): number {
    ...
    if (!Number.isSafeInteger(now) || now < 1) throw new JwksProviderError('JWKS provider clock is invalid');
    return now;
  }
  ...
  private async performRefresh(now: number): Promise<void> {
    ...
    try { document = await this.fetcher.fetchJwks(); }
    catch { throw new JwksProviderError('JWKS refresh failed; refusing stale or unknown signing keys'); }
    this.keys = parseJwks(document, this.maximumKeys);
